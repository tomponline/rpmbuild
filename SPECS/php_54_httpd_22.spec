...
%define apiver	no-debug-non-zts-20100525
...
BuildRequires: curl-devel >= 7.9, httpd >= 2.2.15, libstdc++-devel, openssl-devel, zlib-devel
...
%package apache
Group: Development/Languages
Summary: Apache module of PHP
Requires: php = %{version}-%{release}, httpd >= 2.2.19

%description common
Provides PHP common files

%description apache
Provides PHP Apache module
...
echo "---------BUILIDING PHP-CLI and MOD_PHP-----------"
mkdir build-apache
pushd build-apache
ln -s ../configure
./configure \
...
pushd build-apache
	...
	install -m 755 .libs/libphp5.so $RPM_BUILD_ROOT%{_libdir}/apache/
